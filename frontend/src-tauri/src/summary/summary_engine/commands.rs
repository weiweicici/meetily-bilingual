...
/// Initialize the model manager using canonical SharedModels summary directory
pub async fn init_model_manager<R: Runtime>(app: &AppHandle<R>) -> anyhow::Result<()> {
    ...
    *manager_lock = Some(Arc::new(manager));
    ...
    log::info!("Built-in AI model manager initialized using shared models directory");
    Ok(())
}
...
pub async fn init_model_manager_at_startup<R: Runtime>(
    ...
    *manager_lock = Some(Arc::new(manager));
    ...
    log::info!("ModelManager initialized at startup using shared models directory");
    Ok(())
}
